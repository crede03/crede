...
function handleNetworkStream(url, forcePlay = false, customTitle = null, streamSpeed = null, autoPlay = true) {
    ...
    if (ytMatch && ytMatch[1]) {
        ...
        const initialTitle = customTitle || 'YouTube Stream';
        playlist.push({ file: { name: initialTitle }, title: initialTitle, url: url, isYouTube: true, ytId: ytId, duration: 'Live', folder: 'Network', speed: streamSpeed });
        logSystem("YouTube stream added.");
        renderPlaylist();
        const shouldPlay = forcePlay || (currentTrackIndex === -1 && !ytActive);
        if (shouldPlay) loadTrack(playlist.length - 1, autoPlay);
            ...
            .then(data => {
                if (data.title) {
                    ...
                    if (currentTrackIndex === newIndex) {
                        ...
                        const vTitle = document.getElementById('video-title') || document.getElementById('wmp9-video-title');
                        const plName = document.getElementById('track-name') || document.getElementById('wmp9-playlist-track-name');
                        const activeName = customTitle || data.title;
                        if(vTitle) vTitle.innerText = activeName;
                        if(plName) plName.innerText = activeName;
                    }
                }
            })
...
function loadTrack(index, autoPlay = true) {
    try {
        ...
        if (track.isYouTube) {
            ...
            ytPlayer.src = `https://www.youtube.com/embed/${track.ytId}?autoplay=${autoPlay ? '1' : '0'}&enablejsapi=1&controls=0&disablekb=1&fs=0&modestbranding=1&rel=0&iv_load_policy=3`;
...
window.triggerPopout = function(specificSkin = null) {
    ...
    // Popout uses current local path
...
/* =========================================================================
   WMP CREDE.VIP Desktop Integration
   ========================================================================= */

window.closePlayer = function() {
    try {
        if (window.parent && window.parent !== window && window.parent.document) {
            const win = window.parent.document.getElementById('wmp-window');
            if (win && typeof window.parent.closeWindow === 'function') {
...
window.loadPlaylistFromConfig = async function(autoplay = false) {
    try {
        const res = await fetch('../data/wmp-playlist.json', { cache: 'no-cache' });
        if (res.ok) {
            ...
            if (Array.isArray(list) && list.length > 0) {
                ...
                list.forEach((item, idx) => {
                    ...
                    if (url) {
                        handleNetworkStream(url, idx === 0, title, null, autoplay);
                    }
                });
                return true;
            }
        ...
        console.warn("Could not load wmp-playlist.json", e);
    }
    return false;
};

window.setYouTubePlaylist = function(links, autoplay = true) {
    ...
    links.forEach((item, index) => {
        ...
        const title = typeof item === 'object' ? item.title : null;
        if (url) {
            handleNetworkStream(url, index === 0 && autoplay, title, null, autoplay);
        }
    });
};

// Auto-load playlist from config on launch (staged in paused state, no unwanted audio)
setTimeout(() => {
    window.loadPlaylistFromConfig(false);
}, 250);